...
bool D3D12BaseShader::GetShaderResourceBindings(TD3D12ReflectionInterface* Reflection, D3D12BaseShader* Shader,
	...
	for (uint32 i = 0; i < NumBoundResources; i++) {
		...
		if (!IsLegalRegisterSpace(ShaderBindDesc)) {
			CE_LOG_ERROR(
				"Shader Parameter '" + std::string(ShaderBindDesc.Name) + "' has register space '" + std::to_string(
					ShaderBindDesc.Space) + "' specified, which is invalid.");
			return false;
		}
		...
		if (ShaderBindDesc.Type == D3D_SIT_CBUFFER) {
			...
			if (ShaderBindDesc.Space == D3D12_SHADER_REGISTER_SPACE_32BIT_CONSTANTS) {
				...
				if (ShaderBindDesc.Space == 0) {
					ResourceCount.Ranges.NumCBVs = Math::Max(ResourceCount.Ranges.NumCBVs,
					                                         ShaderBindDesc.BindPoint + ShaderBindDesc.BindCount);
				}
				else {
					RTLocalResourceCount.Ranges.NumCBVs = Math::Max(RTLocalResourceCount.Ranges.NumCBVs,
			...
			if (ShaderBindDesc.Space == 0) {
				ResourceCount.Ranges.NumSamplers = Math::Max(ResourceCount.Ranges.NumSamplers,
				                                             ShaderBindDesc.BindPoint + ShaderBindDesc.BindCount);
			}
			else {
				RTLocalResourceCount.Ranges.NumSamplers = Math::Max(RTLocalResourceCount.Ranges.NumSamplers,
				                                                    ShaderBindDesc.BindPoint + ShaderBindDesc.
				                                                    BindCount);
			}
		}
			...
			if (ShaderBindDesc.Space == 0) {
				ResourceCount.Ranges.NumSRVs = Math::Max(ResourceCount.Ranges.NumSRVs,
				                                         ShaderBindDesc.BindPoint + ShaderBindDesc.BindCount);
			}
			else {
				RTLocalResourceCount.Ranges.NumSRVs = Math::Max(RTLocalResourceCount.Ranges.NumSRVs,
				                                                ShaderBindDesc.BindPoint + ShaderBindDesc.BindCount);
			}
		}
			...
			if (ShaderBindDesc.Space == 0) {
				ResourceCount.Ranges.NumUAVs = Math::Max(ResourceCount.Ranges.NumUAVs,
				                                         ShaderBindDesc.BindPoint + ShaderBindDesc.BindCount);
			}
			else {
				RTLocalResourceCount.Ranges.NumUAVs = Math::Max(RTLocalResourceCount.Ranges.NumUAVs,
				                                                ShaderBindDesc.BindPoint + ShaderBindDesc.BindCount);
			}
		}
	}
...
bool D3D12BaseShader::GetShaderReflection(D3D12BaseShader* Shader) {
	...
	if (!CastDX12ShaderCompiler()->GetReflection(Shader, &Reflection)) {
		return false;
	}
	...
	if (!GetShaderResourceBindings(Reflection.Get(), Shader, ShaderDesc.BoundResources)) {
		CE_LOG_ERROR("[D3D12BaseShader]: Error when analysing shader parameters");
		return false;
	}

	if (CastDX12ShaderCompiler()->HasRootSignature(Shader)) {
...
bool D3D12BaseRayTracingShader::GetRayTracingShaderReflection(D3D12BaseRayTracingShader* Shader) {
	...
	if (!CastDX12ShaderCompiler()->GetLibraryReflection(Shader, &Reflection)) {
		return false;
	}
	...
	if (!GetShaderResourceBindings(Function, Shader, FuncDesc.BoundResources)) {
		CE_LOG_ERROR("[D3D12BaseRayTracingShader]: Error when analysing shader parameters");
		return false;
	}
...
bool D3D12BaseComputeShader::Init() {
	TComPtr<ID3D12ShaderReflection> Reflection;
	if (!CastDX12ShaderCompiler()->GetReflection(this, &Reflection)) {
		return false;
	}
	...
	if (!GetShaderResourceBindings(Reflection.Get(), this, ShaderDesc.BoundResources)) {
		CE_LOG_ERROR("[D3D12BaseComputeShader]: Error when analysing shader parameters");
		return false;
	}

	if (CastDX12ShaderCompiler()->HasRootSignature(this)) {
