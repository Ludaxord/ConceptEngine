...
class CEDX12GraphicsManager : public CEGraphicsManager {
public:
	CEDX12GraphicsManager();
	~CEDX12GraphicsManager();
	...
	virtual class RayTracingScene* CreateRayTracingScene(uint32 Flags, RayTracingGeometryInstance* Instances,
	                                                     uint32 NumInstances) override final;
	virtual class RayTracingGeometry* CreateRayTracingGeometry(uint32 Flags, VertexBuffer* VertexBuffer,
	                                                           IndexBuffer* IndexBuffer) override final;
	...
	CreateUnorderedAccessView(const UnorderedAccessViewCreateInfo& CreateInfo) override final;
	...
	virtual class DepthStencilState*
	CreateDepthStencilState(const DepthStencilStateCreateInfo& CreateInfo) override final;
	...
	virtual class GraphicsPipelineState*
	CreateGraphicsPipelineState(const GraphicsPipelineStateCreateInfo& CreateInfo) override final;
	virtual class ComputePipelineState*
	CreateComputePipelineState(const ComputePipelineStateCreateInfo& CreateInfo) override final;
	virtual class RayTracingPipelineState* CreateRayTracingPipelineState(
		const RayTracingPipelineStateCreateInfo& CreateInfo) override final;
	...
	virtual class Viewport* CreateViewport(CEWindow* Window, uint32 Width, uint32 Height, EFormat ColorFormat,
	                                       EFormat DepthFormat) override final;
...
private:
	...
	bool FinalizeBufferResource(TD3D12Buffer* Buffer, uint32 SizeInBytes, uint32 Flags, EResourceState InitialState,
	                            const ResourceData* InitialData);
