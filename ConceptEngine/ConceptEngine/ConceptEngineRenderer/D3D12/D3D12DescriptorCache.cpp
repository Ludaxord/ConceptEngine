...
bool D3D12DescriptorCache::Init() {
	...
	NullCBV = DBG_NEW D3D12ConstantBufferView(GetDevice(),
	                                          CastDX12GraphicsManager()->GetResourceOfflineDescriptorHeap());
	if (!NullCBV->Init()) {
		return false;
	}
	...
	NullUAV = DBG_NEW D3D12UnorderedAccessView(GetDevice(),
	                                           CastDX12GraphicsManager()->GetResourceOfflineDescriptorHeap());
	if (!NullUAV->Init()) {
		return false;
	}
	...
	NullSRV = DBG_NEW D3D12ShaderResourceView(GetDevice(),
	                                          CastDX12GraphicsManager()->GetResourceOfflineDescriptorHeap());
	if (!NullSRV->Init()) {
		return false;
	}
	...
	NullSampler = DBG_NEW D3D12SamplerState(GetDevice(), CastDX12GraphicsManager()->GetSamplerOfflineDescriptorHeap());
...
void D3D12DescriptorCache::CommitGraphicsDescriptors(D3D12CommandListHandle& CmdList, D3D12CommandBatch* CmdBatch,
...
void D3D12DescriptorCache::CommitComputeDescriptors(D3D12CommandListHandle& CmdList, D3D12CommandBatch* CmdBatch,
                                                    D3D12RootSignature* RootSignature) {
	Assert(CmdBatch != nullptr);
	Assert(RootSignature != nullptr);
