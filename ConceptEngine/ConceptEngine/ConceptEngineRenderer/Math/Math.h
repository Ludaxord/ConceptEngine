...
public:
	...
	inline static std::default_random_engine RandEng;
	inline static std::uniform_int_distribution<int> UniformInt;
	inline static std::uniform_real_distribution<float> UniformFloat;
...
public:
	...
	FORCEINLINE static float RandF() {
		return UniformFloat(RandEng);
	}

	FORCEINLINE static float RandF(float A, float B) {
		return A + UniformFloat(RandEng) * (B - A);
	}

	FORCEINLINE static int Rand(int A, int B) {
		return A + UniformInt(RandEng) % ((B - A) + 1);
	}

	FORCEINLINE static std::string RandStr() {
		return std::to_string(UniformInt(RandEng));
	}
