#include "CEDX12MeshManager.h"

CEDX12MeshManager::CEDX12MeshManager() {
}
...
MeshData CEDX12MeshManager::CreateFromFile(const std::string& Filename, bool MergeMeshes, bool LeftHanded) noexcept {
...
MeshData CEDX12MeshManager::CreateCube(float Width, float Height, float Depth) noexcept {
...
MeshData CEDX12MeshManager::CreatePlane(uint32 Width, uint32 Height) noexcept {
	MeshData data;
	if (Width < 1) {
		Width = 1;
	}
...
MeshData CEDX12MeshManager::CreateSphere(uint32 Subdivisions, float Radius) noexcept {
	MeshData Sphere;
...
MeshData CEDX12MeshManager::CreateCone(uint32 Sides, float Radius, float Height) noexcept {
...
MeshData CEDX12MeshManager::CreatePyramid() noexcept {
	/*
	MeshData data;
...
MeshData CEDX12MeshManager::CreateCylinder(uint32 Sides, float Radius, float Height) noexcept {
	UNREFERENCED_VARIABLE(Sides);
	UNREFERENCED_VARIABLE(Radius);
	UNREFERENCED_VARIABLE(Height);
...
void CEDX12MeshManager::Subdivide(MeshData& OutData, uint32 Subdivisions) noexcept {
	if (Subdivisions < 1) {
		return;
	}
...
void CEDX12MeshManager::Optimize(MeshData& OutData, uint32 StartVertex) noexcept {
...
void CEDX12MeshManager::CalculateHardNormals(MeshData& Data) noexcept {
...
void CEDX12MeshManager::CalculateTangents(MeshData& OutData) noexcept {
