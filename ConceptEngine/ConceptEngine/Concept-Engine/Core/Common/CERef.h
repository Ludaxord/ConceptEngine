...
public:
	...
	CEEnableIf<std::is_convertible_v<TCastType*, T*>, TCastType*> GetAs() const noexcept {
	...
	template <typename TOther>
	CEEnableIf<std::is_convertible_v<TOther*, T*>, bool> operator==(TOther* RHS) const noexcept {
		return (RefPtr == RHS);
	}

	template <typename TOther>
	friend CEEnableIf<std::is_convertible_v<TOther*, T*>, bool> operator==(
		TOther* LHS, const CERef& RHS) noexcept {
		return (RHS == LHS);
	}

	template <typename TOther>
	CEEnableIf<std::is_convertible_v<TOther*, T*>, bool> operator==(
