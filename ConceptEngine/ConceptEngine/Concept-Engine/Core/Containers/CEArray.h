...
namespace ConceptEngine::Core::Containers {
	...
	public:
		...
		explicit CEArray(SizeType Size, const T& Value) noexcept : Array(nullptr), ArraySize(0), ArrayCapacity(0),
		                                                           Allocator() {
			InternalConstruct(Size, Value);
		}

		template <typename TInput>
		explicit CEArray(TInput Begin, TInput End) noexcept: Array(nullptr), ArraySize(0), ArrayCapacity(0),
		                                                     Allocator() {
			InternalConstruct(Begin, End);
		}

		CEArray(std::initializer_list<T> List) noexcept : Array(nullptr), ArraySize(0), ArrayCapacity(0), Allocator() {
		...
		void Fill(T&& Value) noexcept {
			...
			while (ArrayBegin != ArrayEnd) {
				*ArrayBegin = Containers::Move(Value);
				ArrayBegin++;
			}
		}
		...
		T& EmplaceBack(TArgs&&... Args) noexcept {
			...
			new(reinterpret_cast<void*>(DataEnd)) T(Containers::Forward<TArgs>(Args)...);
			ArraySize++;
			return (*DataEnd);
		}
		...
		T& PushBack(T&& Element) noexcept {
			return EmplaceBack(Containers::Move(Element));
		}
		...
		Iterator Emplace(ConstIterator Pos, TArgs&&... Args) noexcept {
			...
			if (Pos == End()) {
				...
				EmplaceBack(Containers::Forward<TArgs>(Args)...);
				return End() - 1;
			}
			...
			new(reinterpret_cast<void*>(DataBegin)) T(Containers::Forward<TArgs>(Args)...);
			ArraySize++;
		...
		Iterator Insert(Iterator Pos, T&& Value) noexcept {
			return Emplace(Pos, Containers::Move(Value));
		}
		...
		Iterator Insert(ConstIterator Pos, std::initializer_list<T> List) noexcept {
			...
			if (Pos == End()) {
				for (const T& Value : List) {
					EmplaceBack(Containers::Move(Value));
				}
		...
		Iterator Append(const CEArray& Other) {
		...
		void Swap(CEArray& Other) noexcept {
			CEArray TempArray(Containers::Move(*this));
			*this = Containers::Move(Other);
			Other = Containers::Move(TempArray);
		}
		...
		CEArray& operator=(const CEArray& Other) noexcept {
		...
		CEArray& operator=(CEArray&& Other) noexcept {
			if (this != std::addressof(Other)) {
				Clear();
				InternalMove(Containers::Forward<CEArray>(Other));
			}

			return *this;
		}

		CEArray& operator=(std::initializer_list<T> List) noexcept {
			Assign(List);
			return *this;
		}
	...
	private:
		// Check that the iterator belongs to this CEArray
		...
		void InternalMove(CEArray&& Other) noexcept {
			InternalReleaseData();
		...
		void InternalMoveEmplace(T* InBegin, T* InEnd, T* Dest) noexcept {
			...
			if constexpr (std::is_trivially_move_constructible<T>()) {
				...
				while (InBegin != InEnd) {
					new(reinterpret_cast<void*>(Dest)) T(Containers::Move(*InBegin));
					InBegin++;
					Dest++;
				}
			}
