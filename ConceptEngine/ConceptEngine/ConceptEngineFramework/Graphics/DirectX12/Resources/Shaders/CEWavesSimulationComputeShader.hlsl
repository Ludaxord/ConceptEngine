/*
 * UpdateWavesCS -> Solves 2D wave equation using compute shader
 *
 * DisturbWavesCS -> Runs one thread to disturb a grid height and its neighbors to generate wave
 */
...
void UpdateWavesCS(int3 dispatchThreadID : SV_DispatchThreadID) {
	/*
	 * We do not need to bounds checking because:
	 * * out-of-bounds read return 0, which works for us -- it just means that boundary of our water simulation is clamped to 0 in local space
