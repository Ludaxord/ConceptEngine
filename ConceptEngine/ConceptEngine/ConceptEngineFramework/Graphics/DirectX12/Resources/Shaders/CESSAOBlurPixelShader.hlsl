...
float4 PS(VertexOut pin) : SV_Target {
	...
	//Center value always contributes to sum
	...
	for (float i = -gBlurRadius; i <= gBlurRadius; ++i) {
		//We already added in center weight
		if (i == 0)
			continue;
		...
		/*
		 * If center value and neighbor values differ too much (either in normal or depth), then we are sampling across a discontinuity.
		 * We discard such samples from blur.
		 */
	...
	//Compensate for discarded samples by making total wights sum to 1.
