...
void CEDirect3D12Graphics::LoadAssets() {
...

	{
		...
		GFX_THROW_INFO(
			D3DCompileFromFile(GetShadersPath(L"shaders.hlsl").c_str(), nullptr, nullptr, "VSMain", "vs_4_0_level_9_3",
				compileFlags, 0, &pVertexShader, nullptr));
		GFX_THROW_INFO(
			D3DCompileFromFile(GetShadersPath(L"shaders.hlsl").c_str(), nullptr, nullptr, "PSMain", "ps_4_0_level_9_3",
				compileFlags, 0, &pPixelShader, nullptr));
		...
		for (auto& i : blendStateDescriptor.RenderTarget)
		...
		// psoDesc.VS = {&vBlob[0], vBlob->GetBufferSize()};
		// psoDesc.PS = {&pBlob[0], pBlob->GetBufferSize()};
		...
		// pDevice->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_pipelineState));
	}
