...
#include "../../Libs/Headers/CEWinLib.h"
...
using WindowMap = std::map<HWND, std::weak_ptr<CEWindow>>;
using WindowMapByName = std::map<std::wstring, std::weak_ptr<CEWindow>>;
 ...
 */
struct CEWindowInstance : public CEWindow {
	CEWindowInstance(HWND hWnd, const std::wstring& windowName, int clientWidth, int clientHeight) : CEWindow(
...
std::shared_ptr<CEWindow> CEGame::CreateWindow(const std::wstring& windowName, int clientWidth, int clientHeight) {
	...
	auto pWindow = std::make_shared<CEWindowInstance>(hWindow, windowName, clientWidth, clientHeight);
...
std::shared_ptr<CEWindow> CEGame::GetWindowByName(const std::wstring& windowName) const {
	...
	while (!m_bTerminateDirectoryChangeThread) {
		...
		case WAIT_OBJECT_0:
			...
			if (m_directoryChanges.CheckOverflow()) {
				...
				Actions::FileAction fileAction = Actions::FileAction::Unknown;
				switch (action) {
				case FILE_ACTION_ADDED:
					fileAction = Actions::FileAction::Added;
					break;
				case FILE_ACTION_REMOVED:
					fileAction = Actions::FileAction::Removed;
					break;
				case FILE_ACTION_MODIFIED:
					fileAction = Actions::FileAction::Modified;
					break;
				case FILE_ACTION_RENAMED_OLD_NAME:
					fileAction = Actions::FileAction::RenameOld;
					break;
				case FILE_ACTION_RENAMED_NEW_NAME:
					fileAction = Actions::FileAction::RenameNew;
					break;
				default:
					break;
				}
				Actions::FileChangedEventArgs fileChangedEventArgs(fileAction, fileName);
 ...
 */
static Actions::MouseButton DecodeMouseButton(UINT messageID) {
	Actions::MouseButton mouseButton = Actions::MouseButton::None;
	switch (messageID) {
	case WM_LBUTTONDOWN:
	case WM_LBUTTONUP:
	case WM_LBUTTONDBLCLK: {
		mouseButton = Actions::MouseButton::Left;
	}
	break;
	case WM_RBUTTONDOWN:
	case WM_RBUTTONUP:
	case WM_RBUTTONDBLCLK: {
		mouseButton = Actions::MouseButton::Right;
	}
	break;
	case WM_MBUTTONDOWN:
	case WM_MBUTTONUP:
	case WM_MBUTTONDBLCLK: {
		mouseButton = Actions::MouseButton::Middle;
	}
	break;
	}
	return mouseButton;
 ...
 */
static Actions::ButtonState DecodeButtonState(UINT messageID) {
	Actions::ButtonState buttonState = Actions::ButtonState::Pressed;
	switch (messageID) {
	case WM_LBUTTONUP:
	case WM_RBUTTONUP:
	case WM_MBUTTONUP:
	case WM_XBUTTONUP:
		buttonState = Actions::ButtonState::Released;
		break;
	case WM_LBUTTONDOWN:
	case WM_RBUTTONDOWN:
	case WM_MBUTTONDOWN:
	case WM_XBUTTONDOWN:
		buttonState = Actions::ButtonState::Pressed;
		break;
	}
 ...
 */
static Actions::WindowState DecodeWindowState(WPARAM wParam) {
	Actions::WindowState windowState = Actions::WindowState::Restored;

	switch (wParam) {
	case SIZE_RESTORED:
		windowState = Actions::WindowState::Restored;
		break;
	case SIZE_MINIMIZED:
		windowState = Actions::WindowState::Minimized;
		break;
	case SIZE_MAXIMIZED:
		windowState = Actions::WindowState::Maximized;
		break;
	default:
		break;
	}
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	std::shared_ptr<CEWindow> pWindow;
	{
	...
	if (pWindow) {
		...
		case WM_DPICHANGED: {
			...
			Actions::DPIScaleEventArgs dpiScaleEventArgs(dpiScaling);
		...
		case WM_PAINT: {
			 ...
			 */
			Actions::UpdateEventArgs updateEventArgs(0.0, 0.0);
		...
		case WM_KEYDOWN: {
			...
			Actions::KeyEventArgs keyEventArgs(key, c, Actions::KeyState::Pressed, control, shift, alt);
		...
		case WM_KEYUP: {
			...
			Actions::KeyEventArgs keyEventArgs(key, c, Actions::KeyState::Released, control, shift, alt);
		...
		case WM_MOUSEMOVE: {
			...
			Actions::MouseMotionEventArgs mouseMotionEventArgs(lButton, mButton, rButton, control, shift, x, y);
		...
		case WM_MBUTTONDOWN: {
			...
			Actions::MouseButtonEventArgs mouseButtonEventArgs(DecodeMouseButton(message),
			                                                   Actions::ButtonState::Pressed, lButton,
		...
		case WM_MBUTTONUP: {
			...
			Actions::MouseButtonEventArgs mouseButtonEventArgs(DecodeMouseButton(message),
			                                                   Actions::ButtonState::Released, lButton,
			                                                   mButton, rButton, control, shift, x, y);
		...
		case WM_MOUSEWHEEL: {
			...
			Actions::MouseWheelEventArgs mouseWheelEventArgs(zDelta, lButton, mButton, rButton, control, shift,
			                                                 (int)screenToClientPoint.x, (int)screenToClientPoint.y);
			pWindow->OnMouseWheel(mouseWheelEventArgs);
		}
		break;
		case WM_CAPTURECHANGED: {
			Actions::EventArgs mouseBlurEventArgs;
			pWindow->OnMouseBlur(mouseBlurEventArgs);
		}
		break;
		case WM_MOUSEACTIVATE: {
			Actions::EventArgs mouseFocusEventArgs;
			pWindow->OnMouseFocus(mouseFocusEventArgs);
		}
		break;
		case WM_MOUSELEAVE: {
			Actions::EventArgs mouseLeaveEventArgs;
			pWindow->OnMouseLeave(mouseLeaveEventArgs);
		}
		break;
		case WM_SIZE: {
			Actions::WindowState windowState = DecodeWindowState(wParam);

			int width = ((int)(short)LOWORD(lParam));
			int height = ((int)(short)HIWORD(lParam));

			Actions::ResizeEventArgs resizeEventArgs(width, height, windowState);
			pWindow->OnResize(resizeEventArgs);
		}
		break;
		case WM_CLOSE: {
			Actions::WindowCloseEventArgs windowCloseEventArgs;
