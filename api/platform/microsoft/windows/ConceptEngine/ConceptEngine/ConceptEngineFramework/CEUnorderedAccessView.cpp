...
CEUnorderedAccessView::CEUnorderedAccessView(CEDevice& device, const std::shared_ptr<CEResource>& resource,
                                             ...
                                             const D3D12_UNORDERED_ACCESS_VIEW_DESC* uav): m_device(device),
	m_resource(resource), m_counterResource(counterResource) {
	...
	if (m_resource) {
		...
		/*
		 * Resource must be created with D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS flag;
		 */
