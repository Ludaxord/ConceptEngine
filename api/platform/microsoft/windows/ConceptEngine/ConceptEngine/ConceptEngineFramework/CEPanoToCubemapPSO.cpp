...
CEPanoToCubemapPSO::CEPanoToCubemapPSO(CEDevice& device) {
	...
	CD3DX12_DESCRIPTOR_RANGE1 destMip(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 5, 0, 0,
	                                  D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE);
	...
	rootParameters[PanoToCubemapRS::PanoToCubemapCB].InitAsDescriptorTable(1, &srcMip);
	rootParameters[PanoToCubemapRS::PanoToCubemapCB].InitAsDescriptorTable(1, &destMip);

	CD3DX12_STATIC_SAMPLER_DESC linearRepeatSampler(0, D3D12_FILTER_MIN_MAG_MIP_LINEAR, D3D12_TEXTURE_ADDRESS_MODE_WRAP,
	                                                D3D12_TEXTURE_ADDRESS_MODE_WRAP, D3D12_TEXTURE_ADDRESS_MODE_WRAP);

	CD3DX12_VERSIONED_ROOT_SIGNATURE_DESC rootSignatureDesc(PanoToCubemapRS::NumRootParameters, rootParameters, 1,
	                                                        &linearRepeatSampler);
	m_rootSignature = device.CreateRootSignature(rootSignatureDesc.Desc_1_1);

	/*
	 * Create PSO for GenerateMips shader
	 */
	...
	auto panoToCubemap = CEPipelineStateObject::CompileShaderFileToByteCode(L"PanoToCubemap_CS.hlsl");
	pipelineStateStream.pRootSignature = m_rootSignature->GetD3D12RootSignature().Get();
	pipelineStateStream.CS = {panoToCubemap.pShaderBytecode, panoToCubemap.BytecodeLength};
	...
	UINT descriptorHandleIncrementSize = device.
