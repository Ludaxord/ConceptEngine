    ...
    , m_pipelineState(nullptr)
{
    ...
    for (int i = 0; i < D3D12_DESCRIPTOR_HEAP_TYPE_NUM_TYPES; ++i)
    {
        m_dynamicDescriptorHeap[i] =
            std::make_unique<CEDynamicDescriptorHeap>(device, static_cast<D3D12_DESCRIPTOR_HEAP_TYPE>(i));
...
void CECommandList::ResolveSubResource(const std::shared_ptr<CEResource>& dstRes, const std::shared_ptr<CEResource>& srcRes,
...
ComPtr<ID3D12Resource> CECommandList::CopyBuffer(size_t bufferSize, const void* bufferData, D3D12_RESOURCE_FLAGS flags)
{
...
std::shared_ptr<CEScene> CECommandList::CreateCone(float radius, float height, uint32_t tessellation, bool reverseWinding)
{
    if (tessellation < 3)
        throw std::out_of_range("tessellation parameter out of range");
...
void CECommandList::SetGraphics32BitConstants(uint32_t rootParameterIndex, uint32_t numConstants, const void* constants)
{
    m_commandList->SetGraphicsRoot32BitConstants(rootParameterIndex, numConstants, constants, 0);
}

void CECommandList::SetCompute32BitConstants(uint32_t rootParameterIndex, uint32_t numConstants, const void* constants)
{
