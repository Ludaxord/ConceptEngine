...
class CED3D12Device : public CEDevice {

public:
	CED3D12Device(std::shared_ptr<CEAdapter> adapter) : CEDevice(adapter) {
	}

	virtual ~CED3D12Device() = default;
	...
	CECommandQueue& GetCommandQueue(CEType type) override;
