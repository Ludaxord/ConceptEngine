...
#include "CEDirectXVertexTypes.h"
...
namespace ConceptEngine::GraphicsEngine::DirectX {
	...
	class CEDirectXCommandList : public std::enable_shared_from_this<CEDirectXCommandList> {
	public:
		/*
		 ...
		 */
		CEDirectXDevice& GetDevice() const {
			return m_device;
		}
		...
		*/
		void TransitionBarrier(const std::shared_ptr<CEDirectXResource>& resource, D3D12_RESOURCE_STATES stateAfter,
		 ...
		 * Resource barriers need to be flushed before a command (draw, dispatch or copy) that expects the resource to be in a particular state can run.
 		 */
		void UAVBarrier(const std::shared_ptr<CEDirectXResource>& resource = nullptr, bool flushBarriers = false);
		 ...
		 */
		void AliasingBarrier(const std::shared_ptr<CEDirectXResource>& = nullptr,
		                     const std::shared_ptr<CEDirectXResource>& afterResource = nullptr,
		 ...
		 * Copy Resources
		 */
		void CopyResource(const std::shared_ptr<CEDirectXResource>& destinationResource,
		                  const std::shared_ptr<CEDirectXResource>& sourceResource);
		 ...
		 */
		void ResolveSubResource(const std::shared_ptr<CEDirectXResource>&, const std::shared_ptr<CEDirectXResource>&,
		                        uint32_t destinationSubResource = 0, uint32_t sourceSubResource = 0);

		/*
		 * Copy content to a vertex buffer in GPU Memory;
		 */
		std::shared_ptr<CEDirectXVertexBuffer> CopyVertexBuffer(size_t numVertices, size_t vertexStride,
		                                                        const void* vertexBufferData);

		template <typename T>
		std::shared_ptr<CEDirectXVertexBuffer> CopyVertexBuffer(const std::vector<T>& vertexBufferData) {
		 ...
		 */
		std::shared_ptr<CEDirectXTexture> LoadTextureFromFile(const std::wstring& fileName, bool sRGB = false);
		 ...
		 */
		std::shared_ptr<CEDirectXScene> LoadSceneFromFile(const std::wstring& fileName,
		 ...
		 */
		std::shared_ptr<CEDirectXScene> LoadSceneFromString(const std::string& sceneString, const std::string& format);
		 ...
		 */
		std::shared_ptr<CEDirectXScene> CreateCube(float size = 1.0, bool reverseWinding = false);
		 ...
		 */
		std::shared_ptr<CEDirectXScene> CreateSphere(float radius = 0.5f, uint32_t tesselation = 16,
		 ...
		 */
		std::shared_ptr<CEDirectXScene> CreateCylinder(float radius = 0.5f, float height = 1.0f,
		 ...
		 * @param reverseWinding, Whether to reverse winding order of triangles.
		 */
		std::shared_ptr<CEDirectXScene> CreateCone(float radius = 0.5f, float height = 1.0f, uint32_t tessellation = 32,
		                                           bool reverseWinding = false);
		 ...
		 */
		std::shared_ptr<CEDirectXScene> CreateQuad(float size = 1.0, bool reverseWinding = false);
		 ...
		 */
		std::shared_ptr<CEDirectXScene> CreateTriangle(float size = 1.0, bool reverseWinding = false);
		 ...
		 */
		std::shared_ptr<CEDirectXScene> CreateTorus(float radius = 0.5f, float thickness = 0.333f,
		                                            uint32_t tessellation = 32, bool reverseWinding = false);
		 ...
		 */
		std::shared_ptr<CEDirectXScene> CreatePlane(float width = 1.0f, float height = 1.0f,
		                                            bool reverseWinding = false);
		 ...
		 */
		std::shared_ptr<CEDirectXScene> CreateCar(float width = 1.0f, float height = 1.0f,
		                                          bool reverseWinding = false);

		/*
		 * Clear texture
		 */
		void ClearTexture(const std::shared_ptr<CEDirectXTexture>& texture, const float clearColor[4]);

		/*
		 * Clear depth/stencil texture
		 */
		void ClearDepthStencilTexture(const std::shared_ptr<CEDirectXTexture>& texture, D3D12_CLEAR_FLAGS clearFlags,
		 ...
		 */
		void GenerateMips(const std::shared_ptr<CEDirectXTexture> texture);

		/*
		 * Generate cubeMap texture from panoramic (EquiRectangular) texture;
		 */
		void PanoToCubeMap(const std::shared_ptr<CEDirectXTexture>& cubeMapTexture,
		                   const std::shared_ptr<CEDirectXTexture>* panoTexture);

		/*
		 * Copy SubResource data to texture;
		 */
		void CopyTextureSubResource(const std::shared_ptr<CEDirectXTexture>& texture, uint32_t firstSubResource,
		...
		void SetVertexBuffers(uint32_t startSlot,
		                      const std::vector<std::shared_ptr<CEDirectXVertexBuffer>>& vertexBufferViews);
		void SetVertexBuffer(uint32_t slot, const std::shared_ptr<CEDirectXVertexBuffer>& vertexBufferView);
		 ...
		 */
		void SetIndexBuffer(const std::shared_ptr<CEDirectXIndexBuffer>& indexBuffer);
		 ...
		 */
		void SetPipelineState(const std::shared_ptr<CEDirectXPipelineStateObject>& pipelineState);

		/*
		 * Set the current root signature on the command list;
		 */
		void SetGraphicsRootSignature(const std::shared_ptr<CEDirectXRootSignature>& rootSignature);
		void SetComputeRootSignature(const std::shared_ptr<CEDirectXRootSignature>& rootSignature);
		 ...
		 */
		void SetConstantBufferView(uint32_t rootParameterIndex, const std::shared_ptr<CEDirectXConstantBuffer>& buffer,
		...
		void SetConstantBufferView(uint32_t rootParameterIndex, uint32_t descriptorOffset,
		                           const std::shared_ptr<CEDirectXConstantBufferView>& cbv,
		                           D3D12_RESOURCE_STATES stateAfter = D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
		...
		void SetShaderResourceView(uint32_t rootParameterIndex, uint32_t descriptorOffset,
		                           const std::shared_ptr<CEDirectXShaderResourceView>& srv,
		                           D3D12_RESOURCE_STATES stateAfter = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE |
		...
		void SetShaderResourceView(int32_t rootParameterIndex, uint32_t descriptorOffset,
		                           const std::shared_ptr<CEDirectXTexture>& texture,
		                           D3D12_RESOURCE_STATES stateAfter = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE |
			                           D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
		...
		void SetUnorderedAccessView(uint32_t rootParameterIndex, uint32_t descriptorOffset,
		                            const std::shared_ptr<CEDirectXUnorderedAccessView>& uav,
		                            D3D12_RESOURCE_STATES stateAfter = D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
		                            UINT firstSubResource = 0,
		                            UINT numSubResources = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES);
		 ...
		 */
		void SetUnorderedAccessView(uint32_t rootParameterIndex, uint32_t descriptorOffset,
		                            const std::shared_ptr<CEDirectXTexture>& texture, UINT mip,
		                            D3D12_RESOURCE_STATES stateAfter = D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
		                            UINT firstSubResource = 0,
		                            UINT numSubResources = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES);
		 ...
		 */
		void SetRenderTarget(const CEDirectXRenderTarget& renderTarget);
	...
	protected:
		friend class CEDirectXCommandQueue;
		friend class CEDirectXDynamicDescriptorHeap;
		friend class std::default_delete<CEDirectXCommandList>;

		CEDirectXCommandList(CEDirectXDevice& device, D3D12_COMMAND_LIST_TYPE type);
		~CEDirectXCommandList();
		 ...
		 */
		bool Close(const std::shared_ptr<CEDirectXCommandList>& pendingCommandList);
		/*
		 ...
		 * Before command list is returned from CEDirectXCommandQueue::GetCommandList.
		 */
		void Reset();
	...
	private:
		 ...
		 */
		using VertexCollection = std::vector<CEDirectXVertexPositionNormalTangentBitangentTexture>;
		using IndexCollection = std::vector<uint16_t>;

		/*
		 * Create a scene that contains single node with a single mesh
		 */
		std::shared_ptr<CEDirectXScene> CreateScene(const VertexCollection& vertices,
