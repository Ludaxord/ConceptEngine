...
bool CERayTracingPlayground::LoadContent() {
	...
	m_deviceResources->SetWindow(m_window->GetWindowHandle(), m_width, m_height);
...
void CERayTracingPlayground::InitializeScene() {
...
void CERayTracingPlayground::CreateDeviceDependentResources() {
...
void CERayTracingPlayground::CreateWindowSizeDependentResources() {
	CreateRaytracingOutputResource();
	UpdateCameraMatrices();
}

void CERayTracingPlayground::UpdateCameraMatrices() {
...
void CERayTracingPlayground::CreateRaytracingInterfaces() {
...
void CERayTracingPlayground::CreateRootSignatures() {
	auto device = m_deviceResources->GetD3DDevice();
	...
	// This is a root signature that enables a shader to have unique arguments that come from shader tables.
	{
		// Triangle geometry
		{
			...
			SerializeAndCreateRaytracingRootSignature(localRootSignatureDesc,
			                                          &m_raytracingLocalRootSignature[LocalRootSignature::Type::Triangle
			                                          ]);
		}
		...
		// AABB geometry
		{
			...
			localRootSignatureDesc.Flags = D3D12_ROOT_SIGNATURE_FLAG_LOCAL_ROOT_SIGNATURE;
			SerializeAndCreateRaytracingRootSignature(localRootSignatureDesc,
			                                          &m_raytracingLocalRootSignature[LocalRootSignature::Type::AABB]);
		}
	}
}

void CERayTracingPlayground::CreateRaytracingPipelineStateObject() {
...
void CERayTracingPlayground::CreateDescriptorHeap() {
	auto device = m_deviceResources->GetD3DDevice();
...
void CERayTracingPlayground::BuildGeometry() {
	BuildProceduralGeometryAABBs();
	BuildPlaneGeometry();
}

void CERayTracingPlayground::BuildAccelerationStructures() {
	auto device = m_deviceResources->GetD3DDevice();
	auto commandList = m_deviceResources->GetCommandList();
...
void CERayTracingPlayground::CreateConstantBuffers() {
	auto device = m_deviceResources->GetD3DDevice();
...
void CERayTracingPlayground::CreateAABBPrimitiveAttributesBuffers() {
	auto device = m_deviceResources->GetD3DDevice();
	auto frameCount = m_deviceResources->GetBackBufferCount();
	m_aabbPrimitiveAttributeBuffer.Create(device, IntersectionShaderType::TotalPrimitiveCount, frameCount,
	                                      L"AABB primitive attributes");
}

void CERayTracingPlayground::BuildShaderTables() {
	auto device = m_deviceResources->GetD3DDevice();
	...
	// Hit group shader table.
	{
		...
		// Triangle geometry hit groups.
		{
			...
			for (auto& hitGroupShaderID : hitGroupShaderIDs_TriangleGeometry) {
				hitGroupShaderTable.
					push_back(ShaderRecord(hitGroupShaderID, shaderIDSize, &rootArgs, sizeof(rootArgs)));
			}
		}
...
void CERayTracingPlayground::CreateRaytracingOutputResource() {
...
void CERayTracingPlayground::SerializeAndCreateRaytracingRootSignature(D3D12_ROOT_SIGNATURE_DESC& desc,
                                                                       ComPtr<ID3D12RootSignature>* rootSig) {
	auto device = m_deviceResources->GetD3DDevice();
	ComPtr<ID3DBlob> blob;
	ComPtr<ID3DBlob> error;
	D3D12SerializeRootSignature(&desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob, &error);
	auto err = error ? static_cast<wchar_t*>(error->GetBufferPointer()) : nullptr;
	ThrowIfFailed(device->CreateRootSignature(1, blob->GetBufferPointer(), blob->GetBufferSize(),
	                                          IID_PPV_ARGS(&(*rootSig))));

}

void CERayTracingPlayground::CreateDxilLibrarySubobject(CD3DX12_STATE_OBJECT_DESC* raytracingPipeline) {
...
void CERayTracingPlayground::CreateHitGroupSubobjects(CD3DX12_STATE_OBJECT_DESC* raytracingPipeline) {
	// Triangle geometry hit groups
	{
...
void CERayTracingPlayground::CreateLocalRootSignatureSubobjects(CD3DX12_STATE_OBJECT_DESC* raytracingPipeline) {
...
void CERayTracingPlayground::BuildProceduralGeometryAABBs() {
	auto device = m_deviceResources->GetD3DDevice();
...
void CERayTracingPlayground::BuildPlaneGeometry() {
	auto device = m_deviceResources->GetD3DDevice();
	// Plane indices.
	Index indices[] =
	{
...
UINT CERayTracingPlayground::CreateBufferSRV(D3DBuffer* buffer, UINT numElements, UINT elementSize) {
	auto device = m_deviceResources->GetD3DDevice();
...
UINT CERayTracingPlayground::AllocateDescriptor(D3D12_CPU_DESCRIPTOR_HANDLE* cpuDescriptor, UINT descriptorIndexToUse) {
...
void CERayTracingPlayground::BuildGeometryDescsForBottomLevelAS(
	std::array<std::vector<D3D12_RAYTRACING_GEOMETRY_DESC>, BottomLevelASType::Count>& geometryDescs) {
	...
	// AABB geometry desc
	{
		...
		for (UINT i = 0; i < IntersectionShaderType::TotalPrimitiveCount; i++) {
			...
			geometryDesc.AABBs.AABBs.StartAddress = m_aabbBuffer.resource->GetGPUVirtualAddress() + i * sizeof(
				D3D12_RAYTRACING_AABB);
		}
	}
}

AccelerationStructureBuffers CERayTracingPlayground::BuildBottomLevelAS(
...
AccelerationStructureBuffers CERayTracingPlayground::BuildTopLevelAS(AccelerationStructureBuffers bottomLevelAS[2],
...
void CERayTracingPlayground::BuildBotomLevelASInstanceDescs(BLASPtrType* bottomLevelASaddresses,
                                                            ComPtr<ID3D12Resource>* instanceDescsResource) {
