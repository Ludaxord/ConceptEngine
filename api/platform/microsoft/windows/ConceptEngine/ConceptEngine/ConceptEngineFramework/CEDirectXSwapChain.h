...
namespace ConceptEngine::GraphicsEngine::DirectX {
	...
	public:
		 ...
		 */
		const CEDirectXRenderTarget& GetRenderTarget() const;
		 ...
		 * Use CEDirectXSwapChain::RenderTarget to get render target for window color buffer
		 * @returns current Back Buffer index after present;
		 */
		UINT Present(const std::shared_ptr<CEDirectXTexture>& texture = nullptr);
	...
	protected:
		CEDirectXSwapChain(CEDirectXDevice& device, HWND hWnd,
		                   DXGI_FORMAT renderTargetFormat = DXGI_FORMAT_R10G10B10A2_UNORM);
		virtual ~CEDirectXSwapChain();
		/*
	...
	private:
		...
		*/
		CEDirectXCommandQueue& m_commandQueue;
		wrl::ComPtr<IDXGISwapChain4> m_dxgiSwapChain;
		std::shared_ptr<CEDirectXTexture> m_backBufferTextures[BufferCount];
		mutable CEDirectXRenderTarget m_renderTarget;
