...
CEGenerateMipsPSO::CEGenerateMipsPSO(CEDevice& device) {
	...
	/*
	 * Create PSO for generate  mips shader;
	 */
	...
	auto generatedMips = CEPipelineStateObject::CompileShaderFileToByteCode(L"GenerateMips_CS.hlsl");
	pipelineStateStream.pRootSignature = m_rootSignature->GetD3D12RootSignature().Get();
	pipelineStateStream.CS = {
		generatedMips.pShaderBytecode,
		generatedMips.BytecodeLength
	};

	m_pipelineState = device.CreatePipelineStateObject(pipelineStateStream);

	/*
	 * Create default texture UAV to pad any unused UAV during mip map generation.
	 */
