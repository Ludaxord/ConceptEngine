...
CEUploadBuffer::Allocation CEUploadBuffer::Allocate(size_t sizeInBytes, size_t alignment) {
	...
	/*
	 * If there is no current page, or requested allocation exceeds remaining space in current page, request new page. 
	 */
...
void CEUploadBuffer::Reset() {
	...
	for (auto page : m_availablePages) {
		/*
		 * Reset page for new allocations.
		 */
		page->Reset();
	}
}

CEUploadBuffer::CEUploadBuffer(CEDevice& device, size_t pageSize): m_device(device), m_pageSize(pageSize) {
}

CEUploadBuffer::~CEUploadBuffer() {
}

CEUploadBuffer::Page::Page(CEDevice& device, size_t sizeInBytes): m_device(device), m_pageSize(sizeInBytes),
                                                                  m_offset(0), m_CPUPointer(nullptr),
...
bool CEUploadBuffer::Page::HasSpace(size_t sizeInBytes, size_t alignment) const {
	size_t alignedSize = Concept::Math::AlignUp(sizeInBytes, alignment);
	size_t alignedOffset = Concept::Math::AlignUp(m_offset, alignment);
...
CEUploadBuffer::Allocation CEUploadBuffer::Page::Allocate(size_t sizeInBytes, size_t alignment) {
	if (!HasSpace(sizeInBytes, alignment)) {
		/*
		 * Can not allocate space from page
		 */
		throw std::bad_alloc();
	}
	size_t alignedSize = Concept::Math::AlignUp(sizeInBytes, alignment);
	m_offset = Concept::Math::AlignUp(m_offset, alignment);
