...
namespace ConceptEngine::GraphicsEngine::Direct3D12 {
	...
	class CEDirectXDynamicDescriptorHeap {
	public:
		CEDirectXDynamicDescriptorHeap(CEDirectXDevice& device, D3D12_DESCRIPTOR_HEAP_TYPE heapType,
		                               uint32_t numDescriptorsPerHeap = 1024);
		virtual ~CEDirectXDynamicDescriptorHeap();
		...
		void CommitStagedDescriptorsForDraw(CEDirectXCommandList& commandList);
		void CommitStagedDescriptorsForDispatch(CEDirectXCommandList& commandList);
		 ...
		 */
		D3D12_GPU_DESCRIPTOR_HANDLE CopyDescriptor(CEDirectXCommandList& commandList,
		 ...
		 */
		void ParseRootSignature(const std::shared_ptr<CEDirectXRootSignature>& rootSignature);
	...
	private:
		 ...
		 * Since CEDirectXDynamicDescriptorHeap can not know which function will be used, it must be passed as an argument to function
		 */
		void CommitDescriptorTables(CEDirectXCommandList& commandList,
		                            std::function<void*(ID3D12GraphicsCommandList*, UINT, D3D12_GPU_DESCRIPTOR_HANDLE)>
		                            setFunction);
		void CommitInlineDescriptors(CEDirectXCommandList& commandList,
