...
std::tuple<ID3D12Device*, IDXGIAdapter1*> CEDirect3DGraphics::CreateDeviceAndAdapter(
	...
	while (factory->EnumAdapters1(adapterIndex, &adapter) != DXGI_ERROR_NOT_FOUND) {
		...
		hr = D3D12CreateDevice(adapter, featureLevel, _uuidof(ID3D12Device), nullptr);
		if (SUCCEEDED(hr)) {
			adapterFound = true;
			break;
		}
...
ID3D12Fence* CEDirect3DGraphics::CreateFence(ID3D12Device* device, int initialValue,
                                             D3D12_FENCE_FLAGS fenceFlags) const {
	ID3D12Fence* fence = nullptr;
	device->CreateFence(initialValue, fenceFlags,IID_PPV_ARGS(&fence));
	return fence;
}

DXGI_ADAPTER_DESC CEDirect3DGraphics::GetAdapterDescription(IDXGIAdapter* dxgiAdapter) const {
...
UINT CEDirect3DGraphics::GetDescriptorHandleIncrementSize(ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE heapType) {
	return device->GetDescriptorHandleIncrementSize(heapType);
}

UINT CEDirect3DGraphics::Check4XMSAAQualitySupport(ID3D12Device* device, DXGI_FORMAT backBufferFormat, int sampleCount,
	...
	msQualityLevels.Flags = multisampleFlags;
	msQualityLevels.NumQualityLevels = qualityLevels;
...
std::tuple<ID3D12CommandQueue*, ID3D12CommandAllocator*, ID3D12GraphicsCommandList*>
CEDirect3DGraphics::CreateCommandObjects(ID3D12Device* device) {
	...
	// calling Reset.
	commandList->Close();
	return std::make_tuple(commandQueue, commandAllocator, commandList);
}

IDXGISwapChain* CEDirect3DGraphics::CreateSwapChain(HWND hWnd, IDXGIFactory4* factory,
...
std::tuple<ID3D12DescriptorHeap*, ID3D12DescriptorHeap*> CEDirect3DGraphics::CreateDescriptorHeaps(
...
void CEDirect3DGraphics::UpdateCubes() {
	UpdatePerSecond(1.0);
...
void CEDirect3DGraphics::UpdatePipeline() {
	...
	if (m_Gui->drawFPSCounter) {
		// draw the text
		RenderText(mrRobotFont, std::wstring(L"FPS: ") + std::to_wstring(timer.fps), XMFLOAT2(0.02f, 0.04f),
...
void CEDirect3DGraphics::RenderCubes() {
	if (guiActive) {
		RenderGui();
	}
