...
namespace ConceptEngine::GraphicsEngine::Direct3D12 {
	...
	public:
		CEDirectXResourceStateTracker();
		virtual ~CEDirectXResourceStateTracker();
		...
		void TransitionResource(const CEDirectXResource& resource, D3D12_RESOURCE_STATES stateAfter,
		                        UINT subResource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES);
		 ...
		 */
		void UAVBarrier(const CEDirectXResource* resource = nullptr);
		 ...
		 */
		void AliasBarrier(const CEDirectXResource* resourceBefore = nullptr,
		                  const CEDirectXResource* resourceAfter = nullptr);
		 ...
		 */
		uint32_t FlushPendingResourceBarriers(const std::shared_ptr<CEDirectXCommandList>& commandList);

		/*
		 * Flush any (non-pending) resource barriers that has been pushed to resource state tracker
		 */
		void FlushResourceBarriers(const std::shared_ptr<CEDirectXCommandList>& commandList);
