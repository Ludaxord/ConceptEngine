...
const wchar_t* D3D12RaytracingLibrarySubobjects::c_localRootSignatureAssociationName =
...
void D3D12RaytracingLibrarySubobjects::LoadShaderToBlob() {
	{
		WCHAR assetsPath[512];
		GetAssetsPath(assetsPath, _countof(assetsPath));
		std::wstring m_assetsPath = assetsPath;
		std::wstring lib_assetsPath = m_assetsPath + L"Raytracing.cso";
		ThrowIfFailed(D3DReadFileToBlob(lib_assetsPath.c_str(), &m_libShaderBlob));
	}
}
...
void D3D12RaytracingLibrarySubobjects::SerializeAndCreateRaytracingRootSignature(
...
void D3D12RaytracingLibrarySubobjects::CreateRootSignatures() {
	...
	ThrowIfFailed(device->CreateRootSignature(1, m_libShaderBlob->GetBufferPointer(), m_libShaderBlob->GetBufferSize(),
...
void D3D12RaytracingLibrarySubobjects::CreateRaytracingPipelineStateObject() {
	...
	D3D12_SHADER_BYTECODE libdxil = CD3DX12_SHADER_BYTECODE(m_libShaderBlob.Get());
...
void D3D12RaytracingLibrarySubobjects::BuildAccelerationStructures() {
	...
	AllocateUAVBuffer(
		device, max(topLevelPrebuildInfo.ScratchDataSizeInBytes, bottomLevelPrebuildInfo.ScratchDataSizeInBytes),
...
UINT D3D12RaytracingLibrarySubobjects::AllocateDescriptor(D3D12_CPU_DESCRIPTOR_HANDLE* cpuDescriptor,
